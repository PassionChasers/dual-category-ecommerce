...
@section('title', 'Admin | Designation Management')
...
<div class="flex-1 p-4 md:p-6 bg-gray-50">
    <div class="mb-6 flex justify-between items-center flex-wrap">
        <div class="mb-2 md:mb-0">
            ...
            <p class="text-gray-600">Manage all designations and hierarchy levels</p>
        </div>
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-2 w-full md:w-auto">
            ...
            <button id="new-designation-button"
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between">
            <h3 class="text-lg font-medium text-gray-900">Designation List</h3>
        </div>
            ...
            <table id="taskTable" class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">Designation Name</th>
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">Hierarchy Level</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 flex space-x-2">
                            <button class="edit-btn text-indigo-600 hover:text-indigo-800"
                                {{-- data-id="{{ $designation->id }}" data-name="{{ $designation->designation_name }}"
                                data-level="{{ $designation->hierarchy_level }}" --}}
                                >
                    ...
                    {{-- @empty
                    <tr>
                        <td colspan="4" class="px-4 py-4 text-center text-gray-500">No designations found.</td>
                    </tr>
                    @endforelse --}}
                </tbody>
            </table>
...
<!-- Modal -->
<div id="designation-modal" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-lg shadow-xl transform transition-all max-w-lg w-full p-6 relative">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-4" id="modal-title">New Designation</h3>
            <form id="designation-form" method="POST" class="space-y-4">
                @csrf
...
<script>
    ...
    const newBtn = document.getElementById('new-designation-button');
    const cancelBtn = document.getElementById('cancel-btn');
    const closeBtn = document.getElementById('close-modal-btn');
    const form = document.getElementById('designation-form');
    const modalTitle = document.getElementById('modal-title');
    const methodInput = document.getElementById('form-method');
    const nameInput = document.getElementById('designation-name');
    const levelInput = document.getElementById('designation-level');

    // Open modal for create
    newBtn.addEventListener('click', () => {
        modalTitle.innerText = 'New Designation';
        form.action = "#";
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            modalTitle.innerText = 'Edit Designation';
            form.action = `/designations/${btn.dataset.id}`;
...
// Toast alerts (top-right, auto close after 3s)
@if(session('success'))
Swal.fire({
    toast: true,
    position: 'top-end',
