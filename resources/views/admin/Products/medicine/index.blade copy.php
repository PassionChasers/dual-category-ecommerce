...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeModal = () => { modal.classList.add('hidden'); form.reset(); methodField.value='POST'; idField.value=''; imagePreview.classList.add('hidden'); form.action="{{ route('admin.medicines.store') }}"; };

    overlay.addEventListener('click', closeModal);
    closeModalBtns.forEach(btn=>btn.addEventListener('click',closeModal));

    openCreate.addEventListener('click',()=>{
        modalTitle.innerText='New Medicine'; isActiveField.checked=true; methodField.value='POST'; idField.value=''; form.action="{{ route('admin.medicines.store') }}"; openModal();
    });
    ...
    document.addEventListener('submit',function(e){
        if(e.target.classList.contains('delete-form')){
            e.preventDefault();
            Swal.fire({title:'Are you sure?', text:'This will permanently delete the medicine.', icon:'warning', showCancelButton:true, confirmButtonColor:'#e3342f', cancelButtonColor:'#6c757d', confirmButtonText:'Yes, delete'})
    ...
    const performSearch=()=>{
        ...
        fetch(`{{ route('admin.medicines.index') }}?${params.toString()}`,{headers:{'X-Requested-With':'XMLHttpRequest','Accept':'text/html'}})
        .then(res=>res.text())
        .then(html=>{
