...
@section('title', 'Admin | Task Priority Management')
...
<div class="flex-1 p-4 md:p-6 bg-gray-50">
    <div class="mb-6 flex justify-between items-center flex-wrap">
        <div class="mb-2 md:mb-0">
            <h2 class="text-2xl font-bold text-gray-800">Task Priority Management</h2>
            <p class="text-gray-600">Manage all task priorities and levels</p>
        </div>
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-2 w-full md:w-auto">
            <form method="GET" class="flex flex-wrap w-full gap-2">
                <input type="text" name="search" value="{{ $search }}" placeholder="Search priorities..."
            ...
            <button id="new-priority-button"
                ...
                <i class="fas fa-plus mr-1"></i> New Priority
            </button>
        </div>
    </div>
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden">
        <div class="px-4 py-5 sm:px-6 border-b border-gray-200 flex justify-between">
            <h3 class="text-lg font-medium text-gray-900">Priority List</h3>
        </div>
            ...
            <table id="taskTable" class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left font-semibold text-gray-700">Priority Name</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2 font-semibold text-gray-800">{{ $priority->name }}</td>
                        <td class="px-4 py-2 text-gray-600">{{ $priority->description }}</td>
                        <td class="px-4 py-2 flex space-x-2">
                            <button class="edit-btn text-indigo-600 hover:text-indigo-800" data-id="{{ $priority->id }}"
                                data-name="{{ $priority->name }}" data-description="{{ $priority->description }}">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form method="POST" action="{{ route('task.priority.destroy', $priority) }}"
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-4 py-4 text-center text-gray-500">No priorities found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Modal -->
<div id="priority-modal" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="flex items-center justify-center min-h-screen px-4">
        ...
        <div class="bg-white rounded-lg shadow-xl transform transition-all max-w-lg w-full p-6 relative">
            ...
            <h3 class="text-lg font-medium text-gray-900 mb-4" id="modal-title">New Priority</h3>
            <form id="priority-form" method="POST" class="space-y-4">
                @csrf
                <input type="hidden" id="form-method" name="_method" value="POST">
                <div>
                    <label class="block text-sm font-medium text-gray-700">Priority Name</label>
                    <input type="text" name="name" id="priority-name"
                ...
                <div>
                    ...
                    <textarea name="description" id="priority-description"
...
<script>
    ...
    const newBtn = document.getElementById('new-priority-button');
    ...
    const descInput = document.getElementById('priority-description');

    // Open modal for create
    newBtn.addEventListener('click', () => {
        modalTitle.innerText = 'New Priority';
        form.action = "{{ route('task.priority.store') }}";
    ...
    document.querySelectorAll('.edit-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            modalTitle.innerText = 'Edit Priority';
            form.action = `/task/priority/${btn.dataset.id}`;
