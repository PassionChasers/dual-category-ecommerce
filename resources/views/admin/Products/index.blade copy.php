            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <td class="px-4 py-2" id="priority-badge-{{ $task->id }}">
                            @php
                            $colors = [
                            1 => ['Low', 'bg-green-100 text-green-800'],
                            2 => ['Medium', 'bg-yellow-100 text-yellow-800'],
                            3 => ['High', 'bg-red-100 text-red-800'],
                            ];
                        ...
                        {{-- <td class="px-4 py-2">
                            <select data-task-id="{{ $task->id }}"
                                class="task-priority-select block w-full border border-gray-300 rounded-lg px-2 py-1 text-sm"
                                {{ $task->status == 2 ? 'disabled' : '' }}
                                >
                                <option value="">-- Select Priority --</option>
                                @foreach($priorities as $priority)
                                <option value="{{ $priority->id }}" @if($task->priority_id == $priority->id) selected
                                    @endif>
                                    {{ $priority->name }}
                                </option>
                                @endforeach
...
{{-- Update the priority using AJAX --}}
{{-- @push('scripts') --}}
{{-- <script src="https://cdn.jsdelivr.net/npm/sweetalert2@11"></script> --}}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.task-status-select').forEach(select => {
        select.addEventListener('change', async e => {
            ...
            try {
                const res = await fetch(`{{ url('admin/tasks') }}/${taskId}/status`, {
                    method: 'PATCH',
                    headers: {
    ...
    document.querySelectorAll('.task-priority-select').forEach(select => {
        select.addEventListener('change', async e => {
            ...
            try {
                const res = await fetch(`{{ url('admin/tasks') }}/${taskId}/priority`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
