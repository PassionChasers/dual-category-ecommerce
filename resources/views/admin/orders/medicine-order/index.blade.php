...
@section('title', 'Medicine Order Management')
    ...
    <div class="bg-white shadow rounded-lg overflow-hidden" id="tableData">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse($medicineOrders as $order)
                        <tr>
                            {{-- Serial --}}
                            <td class="px-4 py-2">
                                {{ ($medicineOrders->currentPage() - 1) * $medicineOrders->perPage() + $loop->iteration }}
                            </td>
                            ...
                            {{-- Store --}}
                            <td class="px-4 py-2">
                                {{ $order->medicalstore->Name ?? $order->medicalstore->Name ?? 'N/A' }}
                            </td>
        ...
        <div class="flex flex-col md:flex-row items-center justify-between px-6 py-4 bg-gray-50 border-t">
            <div class="text-sm text-gray-600">
                Showing <strong>{{ $medicineOrders->firstItem() ?? 0 }}</strong> to <strong>{{ $medicineOrders->lastItem() ?? 0 }}</strong> of <strong>{{ $medicineOrders->total() }}</strong> results
            </div>
            <div class="mt-3 md:mt-0">{{ $medicineOrders->links() }}</div>
        </div>
    </div>
</div>
<<<<<<< HEAD
