...
class OrderController extends Controller
{
     ...
     * @param  string $id
     * @param  int $status
     * @param  Closure $callback Additional updates
     * @param  string $message Success message
     */
    private function _updateStatus($id, int $status, \Closure $callback, string $message)
    {
        $order = Order::findOrFail($id);

        \DB::transaction(function () use ($order, $callback, $status) {
            // Set status first
            $order->Status = $status;

            // Run additional updates if provided
            $callback($order);
        });
    ...
    //-----------------
    //Helper method to get online delivery men with caching
    //------------------
    private function getOnlineDeliveryMen()
    {
        // return cache()->remember('delivery_men', 300, function () {

            return DeliveryMan::select([
                    'DeliveryManId',
                    'UserId',
                    'IsOnline'
                ])
    ...
    {
        // return cache()->remember('active_restaurants', 300, function () {
            return Restaurant::select('RestaurantId','Name','Priority')
                ->where('IsActive', true)
    ...
    private function getActiveMedicalStores()
    {
        // return cache()->remember('active_medical_stores', 300, function () {
            return MedicalStore::select('MedicalStoreId','Name','Priority')
                ->where('IsActive', true)
                ->orderBy('Priority', 'asc')
                ->get();
        // });
    ...
    //---------------------
    // Medicine Orders for Admin view
    //---------------------
    public function medicineOrders(Request $request)
    {
        ...
        $allOrders = Order::select([
            ...
            ->whereDoesntHave('items', fn($q) => $q->where('ItemType', 'Food'))
            ->with([
                'items' => function ($q) {
                    $q->select([
                        'OrderItemId',
        ...
        $allDeliveryMan   = $this->getOnlineDeliveryMen();
        $allMedicalStores = $this->getActiveMedicalStores();
        ...
        return view(
            'admin.orders.medicine-order.index',
            compact('allOrders','itemTypes','allMedicalStores','allDeliveryMan')
        );
    }
    ...
    // For medicalstores business view
    public function medicalStoreOrders(Request $request)
    {
        ...
        ->whereIn('BusinessId', $medicalStoreId)
        ->with([
            'items' => function ($q) {
                $q->select([
                    'OrderItemId',
    ...
    // Update order status (by business only) via AJAX
    public function updateStatus(Request $request)
    {
        ...
        $request->validate([
            'order_id' => 'required|exists:Orders,OrderId',
            'status'   => 'required|in:6,7',
        ]);

        $order = Order::findOrFail($request->order_id);

        // Status → API endpoint mapping
        $statusMap = [
            6 => 'preparing',
            7 => 'packed',
        ];

        $endpoint = $statusMap[$request->status] ?? null;
        ...
        if (!$endpoint) {
            return response()->json([
                'success' => false,
                'message' => 'Invalid status for API update'
            ], 422);
        }
        ...
        try {
            ...
            $response = Http::timeout(3)
                ...
                ->post("https://pcsdecom.azurewebsites.net/api/business/orders/{$order->OrderId}/{$endpoint}");
